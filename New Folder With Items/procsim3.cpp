    ...
    for(int i=0;i<schedule_available;i++)
    {
        ...
        if(schedule_queue[i].busy==NOT_BUSY && dispatch_queue.size()!=0)
        {
            ...
            if(scheduling_node1.src_reg[0]!=-1)
            {
                ...
                {
                    schedule_queue[i].src2_tag=register_file[scheduling_node1.src_reg[0]].tag;
...
void execute_instructions()
{
    ...
    for(int i=0; i<schedule_available;i++)
    {
        ...
        {   count0--;
            ...
            if (count1>0 /*&& schedule_queue[i].src1_ready == READY && schedule_queue[i].src2_tag == READY*/ && schedule_queue[i].function_unit==1 && schedule_queue[i].already_executed!=1 && schedule_queue[i].busy==BUSY)
            {   count1--;
                instructions_fired++;
                for(int j=0;j<k0+k1+k2;j++)
                { if(fu[j].valid==NOT_BUSY)
                {
                    fu[j].valid=BUSY;
                    fu[j].dest_tag = schedule_queue[i].dest_tag;
                    fu[j].dest_reg = schedule_queue[i].dest_reg;
            ...
            if (count2>0 && schedule_queue[i].src1_ready == READY && schedule_queue[i].src2_tag == READY && schedule_queue[i].function_unit==2 && schedule_queue[i].already_executed!=1 && schedule_queue[i].busy==BUSY)
                    {   count2--;
                        instructions_fired++;
...
void setup_proc(uint64_t rIn, uint64_t k0In, uint64_t k1In, uint64_t k2In, uint64_t fIn)
{
    ...
    for (int i = 0; i<128; i++)
    {
        register_file[i].ready = NOT_READY;
    }
