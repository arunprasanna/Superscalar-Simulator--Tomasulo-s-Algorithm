...
void add_to_list(linked_list* pointer, common_node* next_node)
{
    pointer->size--;
...
void dispatch_instructions()
{
    ...
    while(dispatch_node!=NULL && schedule_queue.size>0)
    {
        ...
        dispatch_node_temporary->cycle_when_dispatched=cycle;
...
void schedule_cycle1()
{
   // schedule_sort(&schedule_queue);
    ...
    if (pointer_to_schedule!=NULL && function_unit1.available_execution>0 && pointer_to_schedule->src1_tag == READY && pointer_to_schedule->src2_tag == READY && pointer_to_schedule->cycle_when_dispatched==cycle-1 && pointer_to_schedule->p_inst.op_code==0)
    {
            common_node* temporary_node = create_node_for_function_queue(pointer_to_schedule);;
            temporary_node=pointer_to_schedule;
            create_scheduling_node(temporary_node);
       ...
       else if (pointer_to_schedule!=NULL && function_unit2.available_execution>0 && pointer_to_schedule->src1_tag == READY && pointer_to_schedule->src2_tag== READY && pointer_to_schedule->cycle_when_dispatched==cycle-1 && (pointer_to_schedule->p_inst.op_code==1 || pointer_to_schedule->p_inst.op_code==-1))
       ...
       else if (pointer_to_schedule!=NULL && function_unit3.available_execution!=0 && pointer_to_schedule->src1_tag == READY && pointer_to_schedule->src2_tag== READY && pointer_to_schedule->cycle_when_dispatched==cycle-1 && pointer_to_schedule->p_inst.op_code==2)
        {
            printf("\t\t\t\t\nSchedule2 %d\n ", cycle);
            common_node* temporary_node = create_node_for_function_queue(pointer_to_schedule);
            function_unit2.available_execution--;
...
void free_functionunit()
{
    ...
    while(free_node!=NULL)
    {
        ...
        printf("\t cycle when complete %d\t",free_node->cycle_when_complete);
    }
}
...
void removeScheduler()
{
    ...
    {   node_into_scheduler = schedule_queue.head;
        ...
        {  // printf(" \tLOOK HERE %d \t  ", cycle);
            //printf(" \tLOOK HERE %d \t  ", node_into_scheduler->cycle_when_dispatched);
            
            
            if (cycle == (node_into_scheduler->cycle_when_dispatched +1))
            {
