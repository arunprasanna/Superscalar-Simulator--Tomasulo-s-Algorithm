    ...
    for (int i = 0; i<f && flag1==TRUE; i++)
    {
           ...
           printf("\nFetch %d  ", cycle);
            add_to_list(&dispatch_queue, read_variable);
               if(read_variable->line_number==25)
                   flag=0;
...
void dispatch_instructions()
{
    ...
    while(dispatch_node!=NULL && schedule_queue.size>0 && ((dispatch_node->cycle_when_dispatched)<(cycle)))
    {
    {
        create_scheduling_node(dispatch_node);
        dispatch_node_temporary = dispatch_node;
        dispatch_node = dispatch_node->next;
        dispatch_node_temporary->cycle_when_scheduled=cycle+1;
        dispatch_node_temporary->fired=0;
        remove_from_list(&dispatch_queue, dispatch_node_temporary, FALSE);
        add_to_list(&schedule_queue, dispatch_node_temporary);
        printf("\tSchedule %d \t %d", cycle, dispatch_node_temporary->line_number);
    }

    }
    common_node* new_node;
    new_node=schedule_queue.head;
    printf(" \nSIZE %d\n", schedule_queue.size);

    while(new_node!=NULL)
    {   printf("HEllO %d \n",new_node->line_number);
        if(new_node->src1_tag==ALMOST_READY)
        {
            new_node->src1_tag=READY;
            printf("\n\t Helo there %d",new_node->line_number);
            
            
        }
        if(new_node->src2_tag==ALMOST_READY)
        {
            new_node->src2_tag=READY;
        }
...
void schedule_cycle1()
{
   // schedule_sort(&schedule_queue);
    ...
    if (pointer_to_schedule!=NULL && count0>0 && pointer_to_schedule->src1_tag == READY && pointer_to_schedule->src2_tag == READY /*&& pointer_to_schedule->cycle_when_scheduled<cycle*/ && pointer_to_schedule->p_inst.op_code==0 && pointer_to_schedule->fired==0)
    {
            printf(" \t\t\tExecute0 %d ", pointer_to_schedule->line_number);
    ...
    else if (pointer_to_schedule!=NULL && count1>0 && pointer_to_schedule->src1_tag == READY && pointer_to_schedule->src2_tag == READY && pointer_to_schedule->fired==0 /*&& pointer_to_schedule->cycle_when_scheduled<cycle*/ && (pointer_to_schedule->p_inst.op_code==1 || pointer_to_schedule->p_inst.op_code==-1))
        
       {    printf("\t\t\tExecute1 %d ", pointer_to_schedule->line_number);
      ...
      //     printf( "\n Added to FU queue\t\n");
       ...
       else if (pointer_to_schedule!=NULL && count2>0 && pointer_to_schedule->src1_tag == READY && pointer_to_schedule->src2_tag== READY /*&& pointer_to_schedule->cycle_when_scheduled<cycle*/ && pointer_to_schedule->p_inst.op_code==2 && pointer_to_schedule->fired==0)
        {
            printf("\t\t\tExecute2 %d ", pointer_to_schedule->line_number);
    ...
    while(update_node!=NULL)
{
            for (int j = 0;j<r; j++)
            {
                ...
                {
                    update_node->src1_tag=ALMOST_READY;
                    printf("\n\t Reached %d",update_node->line_number);
                }
                if (cdb[j].tag==update_node->src2_tag)
                {
                    update_node->src2_tag= ALMOST_READY;
                    printf("\n\t Reached %d",update_node->line_number);
        ...
        for(node_into_scheduler=schedule_queue.head;node_into_scheduler!=NULL;node_into_scheduler=node_into_scheduler->next)
        {
            for(int i=0;i<r;i++)
            {
                if(cdb[i].tag==node_into_scheduler->dest_tag)
                {
                    ...
                    common_node* node= node_into_scheduler;
                    ...
                    remove_from_list(&schedule_queue, node,FALSE);
...
void setup_proc(uint64_t rIn, uint64_t k0In, uint64_t k1In, uint64_t k2In, uint64_t fIn)
{
    ...
    timer=(timing *) malloc((100000)*sizeof(timing));
