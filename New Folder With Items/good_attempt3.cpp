...
void execute_instructions()
{
    ...
    for(int i=0;i<schedule_available;i++)
    {
        for(int j = 0; j< r; j++)
        {
            if(cdb[j].busy == BUSY && cdb[j].tag == schedule_queue[i].src1_tag && schedule_queue[i].busy==BUSY)
            {
                
                schedule_queue[i].src1_ready = 1;
            }
            else if(cdb[j].busy == BUSY && cdb[j].tag == schedule_queue[i].src2_tag && schedule_queue[i].busy==BUSY)
            {
            ...
            if(cdb[i].busy!=BUSY)
            {
                if(fu[j].busy==BUSY)
                {
                    ...
                    if(fu[j].f_unit==0)
                    {
                        ...
                        printf("\t CDB TAG %d \t LiNE %d\t CYCLE %d \t",cdb[i].tag,cdb[i].line_number,cycle);
