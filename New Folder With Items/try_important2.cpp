...
void schedule_cycle1()
{
   // schedule_sort(&schedule_queue);
    ...
    if (pointer_to_schedule!=NULL && count0>0 && pointer_to_schedule->src1_tag == READY && pointer_to_schedule->src2_tag == READY /*&& pointer_to_schedule->cycle_when_dispatched!=cycle*/ && pointer_to_schedule->p_inst.op_code==0 && pointer_to_schedule->fired==0)
    {
            printf(" \t\t\tSchedule0 %d ", pointer_to_schedule->line_number);
    ...
    else if (pointer_to_schedule!=NULL && count1>0 && pointer_to_schedule->src1_tag == READY && pointer_to_schedule->src2_tag == READY && pointer_to_schedule->fired==0 /*&& pointer_to_schedule->cycle_when_dispatched!=cycle*/ && (pointer_to_schedule->p_inst.op_code==1 || pointer_to_schedule->p_inst.op_code==-1))
        
       {    printf("\t\t\tSchedule1 %d ", pointer_to_schedule->line_number);
      ...
      //     printf( "\n Added to FU queue\t\n");
       ...
       else if (pointer_to_schedule!=NULL && count2>0 && pointer_to_schedule->src1_tag == READY && pointer_to_schedule->src2_tag== READY /*&& pointer_to_schedule->cycle_when_dispatched!=cycle*/ && pointer_to_schedule->p_inst.op_code==2 && pointer_to_schedule->fired==0)
        {
            printf("\t\t\tSchedule2 %d ", pointer_to_schedule->line_number);
